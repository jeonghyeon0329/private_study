...
# 2. GPT-2 모델과 토크나이저 로딩
tokenizer = GPT2Tokenizer.from_pretrained('gpt2')

# pad_token 설정 (GPT-2는 기본적으로 pad_token이 없으므로 eos_token을 pad_token으로 설정)
tokenizer.pad_token = tokenizer.eos_token

# 3. 데이터 전처리 함수 (토큰화 및 레이블 생성)
def tokenize_function(examples):
    # 입력 텍스트와 출력 텍스트 모두를 tokenized하여 반환
    output = tokenizer(examples['dialog'], padding="max_length", truncation=True, max_length=32)
